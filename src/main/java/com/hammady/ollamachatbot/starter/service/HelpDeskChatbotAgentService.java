package com.hammady.ollamachatbot.starter.service;

import com.hammady.ollamachatbot.starter.dto.HistoryEntry;
...
public class HelpDeskChatbotAgentService {
    ...
    If the `user_main_prompt` is not related to internet connectivity issues then just say:
    
    I'm sorry my knowledge only extends to resolving Internet connection issues.
...
""";
    ...
    private static Map<String, List<HistoryEntry>> conversationalHistoryStorage;

    public HelpDeskChatbotAgentService(OllamaChatModel ollamaChatClient) {
        this.ollamaChatClient = ollamaChatClient;
        conversationalHistoryStorage = new HashMap<>();
    }

    public String call(String userMessage, String historyId) {
        var currentHistory = conversationalHistoryStorage.computeIfAbsent(historyId,s ->  new ArrayList<>());
